...
local function open_stash(metadata)
     ...
     TriggerEvent("gh-keepbackpack:client:close", metadata.ID)
end

RegisterNetEvent("gh-keepbackpack:client:open", function(backpack_metadata)
...
RegisterNetEvent("gh-keepbackpack:client:close", function(ID)
...
RegisterNetEvent('gh-keepbackpack:client:enter_password', function(backpack_metadata)
     if backpack_metadata.locked then
          ...
          if inputData then
               ...
               TriggerServerEvent('gh-keepbackpack:server:open_backpack', backpack_metadata)
          end
     else
          TriggerServerEvent('gh-keepbackpack:server:open_backpack', backpack_metadata)
     end
...
RegisterNetEvent('gh-keepbackpack:client:create_password', function(ID)
     local inputData = exports['qb-input']:ShowInput({
          header = 'Password',
          inputs = {
               {
     ...
     if inputData then
          if not inputData.pass then return end
          TriggerServerEvent('gh-keepbackpack:server:add_password', { ID = ID, password = inputData.pass })
     end
end)

RegisterNetEvent("gh-keepbackpack:client:lockpick", function(backpack_metadata)
     local duration = Config.duration.close
     QBCore.Functions.Progressbar("keep_backpack_close", 'Lockpicking', 1 * 1000, false, false, {
          ...
          TriggerServerEvent('gh-keepbackpack:server:open_backpack', backpack_metadata, true)
     end)
end)
