...
RegisterNetEvent('gh-keepbackpack:server:saveBackpack', function(source, stashId, items)
     local Player = QBCore.Functions.GetPlayer(source)
     ...
     QBCore.Functions.CreateUseableItem(item_name, function(source, item)
          ...
          if item.info == '' or (type(item.info) == "table" and item.info.ID == nil) then
               ...
               if value.locked then
                    TriggerClientEvent('gh-keepbackpack:client:create_password', source, metadata.ID)
               end
               return
          end
          ...
          if isOnHotbar(item.slot) then
               ...
               -- fix end
               TriggerClientEvent('gh-keepbackpack:client:enter_password', source, metadata)
          else
...
QBCore.Functions.CreateUseableItem('briefcaselockpicker', function(source, item)
     ...
     for _, name in pairs(picables) do
          ...
          TriggerClientEvent('gh-keepbackpack:client:lockpick', source, metadata)
...
RegisterNetEvent('gh-keepbackpack:server:add_password', function(data)
     local src = source
...
RegisterNetEvent('gh-keepbackpack:server:open_backpack', function(backpack_metadata, lockpick)
     ...
     if not backpack_metadata.locked then
          TriggerClientEvent('gh-keepbackpack:client:open', backpack_metadata.source, safe_data)
          return
     end
